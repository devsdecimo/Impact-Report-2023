...
function IconWithText({
  ...
  const [decimalZero, setDecimalZero] = useState("");
  ...
  const animateCounter2 = (targetNumber) => {
    let currentNumber = parseFloat(targetNumber);
    let start = 0;
    const interval = setInterval(() => {
      start += currentNumber / 100;
      setCounter2(start);
      if (start >= currentNumber) {
        clearInterval(interval);
        setCounter2(currentNumber);
      }
    }, 10);
  };
  ...
  return (
    ...
    >
      ...
      {textBefore && (
        <span className="ml-10 text-before">
          <strong>{textBefore}</strong>
        </span>
      )}
      {numbers && textBefore == "Year discovered: " ? (
        <span
          className={
            textBefore && textBefore !== "~" ? "ml-10" : "ml-10 text-bold"
          }
        >
          {counter.toFixed(toFixed)}
        </span>
      ) : (
        numbers && (
          <span
      ...
      {numbers2 && textBefore == "Year discovered: " ? (
        <span className={textBefore ? "" : "text-bold"}>
          {decimalZero + counter2.toFixed(toFixed)}
        </span>
      ) : (
        numbers2 && (
          <span className={textBefore ? "" : "text-bold"}>
            {decimalZero +
              counter2.toFixed(toFixed).replace(/\B(?=(\d{3})+(?!\d))/g, ",")}
          </span>
        )
      )}
      {text && <span className="ml-10">{text}</span>}
    </IconWithTextContainer>
  );
}
